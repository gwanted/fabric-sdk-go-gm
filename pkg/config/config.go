...
func getCertPool(myViper *viper.Viper) (*sm2.CertPool, error) {
	tlsCertPool := sm2.NewCertPool()
	if myViper.GetBool("client.tlsCerts.systemCertPool") == true {
		var err error
		if tlsCertPool, err = sm2.SystemCertPool(); err != nil {
			return nil, err
		}
